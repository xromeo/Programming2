...
// Good
public class NonConstructibleChain {

    int nonConstructibleChain(int[] coins){
        Arrays.sort(coins);
        int change = 0;
        for (int coin : coins) {
            if(coin > change + 1){
    ...
    public static void main(String[] args) {
        int[] coins = new int[]{1,1,3,4,8,8};
        var chain = new NonConstructibleChain();
        System.out.println(chain.nonConstructibleChain(coins));
    }
}
